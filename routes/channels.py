...
Authorization rules:
  ...
  - Viewing public channels: workspace member (can view even without joining).
...
@channels_bp.route('/workspaces/<int:workspace_id>/channels/create',
                   methods=['GET', 'POST'])
@login_required
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            if cur.fetchone():
                ...
                return render_template('channels/create.html', workspace=workspace,
                                       name=raw_name, channel_type=channel_type)
...
def invite_to_channel(channel_id):
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            if not cur.fetchone():
                flash(f'{invitee_username} must be a workspace member first.', 'warning')
                return render_template('channels/invite.html', channel=channel)
